            ...
            <form action="{{ route('guru.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="id_user" id="id_user" class="form-select" required>
                        ...
                        @foreach ($users as $u)
                            {{-- simpan nama user di data attribute agar mudah diakses JS --}}
                            <option value="{{ $u->id_user }}" data-nama="{{ $u->name }}">
                                {{ $u->name }}
                            </option>
                        @endforeach
                    </select>
...
{{-- Script untuk auto isi nama --}}
<script>
    document.getElementById('id_user').addEventListener('change', function() {
        const selectedOption = this.options[this.selectedIndex];
        const namaUser = selectedOption.getAttribute('data-nama') || '';
